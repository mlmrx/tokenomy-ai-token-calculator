...
const formatNumber = (num: number, precision: number = 2): string => {
    ...
    if (absNum < 1 && absNum !== 0) { if (absNum < 1e-6) { return num.toExponential(precision); } return num.toFixed(precision); }
...
const MemoryCalculator = () => {
  ...
  const handleApplyPreset = useCallback((preset: ModelPreset) => {
      ...
      // Find the ID of the quantization type based on the preset string (e.g., "bf16")
      const matchingQuant = quantizationTypes.find(q => q.id === preset.precision);
      setPrecision(matchingQuant ? matchingQuant.id : 'bf16'); // Use ID, fallback to bf16 ID
  ...
  const handleCopyLink = useCallback(() => {
   ...
   // --- Chart Data Preparation ---
    ...
    const inferenceMemoryBreakdownData = useMemo(() => {
        ...
        return [ { name: "Model Weights", value: inferredWeightsGB }, { name: "Activations", value: inferenceActivationsGB }, { name: "Temp/Overhead", value: inferenceTempGB }, ].filter(item => item && item.value > 0.001);
                       ...
                       <div className="border-b pb-4">
                           ...
                           {memoryFlags.lora && (
                               <div className="mt-2">
                                   ...
                                   {/* Tooltip on Label containing Select */}
                                   <ShadTooltip>
                                        <TooltipTrigger asChild>
                                            {/* Wrap Select in span for Tooltip Trigger */}
                                            <span className="block"> {/* Use block span */}
